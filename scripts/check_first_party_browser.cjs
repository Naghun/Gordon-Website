...
(async()=>{
 ...
 await page.getByRole('button',{name:'Prihvati',exact:true}).waitFor();
 if(events.length)throw Error('Tracking before consent');
 await page.getByRole('button',{name:'Prihvati',exact:true}).click();
 ...
 console.log('Consent, withdrawal, no form-value leakage, single pageview and three article H1 checks passed.');
